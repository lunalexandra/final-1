  ...
  reducers: {
    ...
    addUserInfo: (state, action: PayloadAction<UserInfo>) => {
      ...
      console.log(
        ...
        JSON.parse(JSON.stringify(state.order.user))
      );
    },
    // Добавление места
    addSeat: (
      state,
      action: PayloadAction<{ route: "departure" | "arrival"; seat: SeatInfo }>
    ) => {
      const { route, seat } = action.payload;
      if (route === "departure") {
        state.order.departure.seats.push(seat);
        console.log(
          "Добавлено место для отправления:",
          JSON.parse(JSON.stringify(state.order.departure.seats))
        );
      } else if (state.order.arrival) {
        // Проверяем, есть ли arrival
        state.order.arrival.seats.push(seat);
        console.log(
          "Добавлено место для возвращения:",
          JSON.parse(JSON.stringify(state.order.arrival.seats))
        );
      }
    },
    // Удаление места
    removeSeat: (
      ...
      if (route === "departure") {
        state.order.departure.seats = state.order.departure.seats.filter(
          (seat) => seat.seat_number !== seatNumber
        );
      } else if (state.order.arrival) {
        // Проверяем, есть ли arrival
        state.order.arrival.seats = state.order.arrival.seats.filter(
          (seat) => seat.seat_number !== seatNumber
        );
      }
    },
    ...
    setPassengerValidation: (
      state,
      action: PayloadAction<{ index: number, isValid: boolean }>
    ) => {
      const { index, isValid } = action.payload;
      state.validation[index] = isValid;
      console.log(`сохраняем валидацию для пассажира ${index} ${isValid}`) // Обновляем валидность для конкретного пассажира
    },

    // Проверка, все ли формы валидны
    checkAllValidations: (state) => {
      state.allValid = state.validation.every((isValid) => isValid);
      console.log(`срез вся ваидация ${state.validation} ${state.allValid}`) // Добавьте это свойство в состояние, если хотите
    },
